  ...
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
    <div style="background:#11161f;border:1px solid #1f2937;border-radius:18px;overflow:hidden;">
      <div style="padding:20px 26px;border-bottom:1px solid #1f2937;display:flex;align-items:center;gap:10px;">
        ...
        <strong style="font-size:14px;letter-spacing:0.18em;text-transform:uppercase;color:${escapeHtml(color)};">${escapeHtml(name)}</strong>
      </div>
      <div style="padding:26px;">
        <h1 style="margin:0 0 14px;font-size:22px;color:#ffffff;">${escapeHtml(title)}</h1>
        ${bodyHtml}
      </div>
      <div style="padding:18px 26px;border-top:1px solid #1f2937;font-size:12px;color:#64748b;">
        Email otomatis dari ${escapeHtml(name)}${site ? ` &middot; <a style="color:${escapeHtml(color)};text-decoration:none;" href="${escapeHtml(site)}">${escapeHtml(site.replace(/^https?:\/\//, ''))}</a>` : ''}
      </div>
    </div>
  </div>
</body></html>`;
...
export async function sendVerificationCode(email, code) {
  ...
  const html = shell('Verifikasi Email', `
    ...
    <div style="margin:16px 0;padding:18px;border-radius:14px;background:#020617;border:1px solid ${escapeHtml(brand().color)};text-align:center;font-size:30px;letter-spacing:8px;font-weight:800;color:${escapeHtml(brand().color)};">${escapeHtml(code)}</div>
...
export async function sendInvoiceCreated(email, invoice) {
  ...
  const html = shell('Invoice Langganan Dibuat', `
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px;color:#e2e8f0;">
      <tr><td style="padding:6px 0;color:#94a3b8;">ID</td><td style="padding:6px 0;text-align:right;"><code style="color:#cbd5f5;">${escapeHtml(invoice.id)}</code></td></tr>
      <tr><td style="padding:6px 0;color:#94a3b8;">Paket</td><td style="padding:6px 0;text-align:right;">${escapeHtml(invoice.label)}</td></tr>
      <tr><td style="padding:6px 0;color:#94a3b8;">Metode</td><td style="padding:6px 0;text-align:right;">${escapeHtml(invoice.method.toUpperCase())}</td></tr>
    ...
    <p style="margin:14px 0 0;color:#94a3b8;font-size:13px;">${escapeHtml(invoice.instructions || '')}</p>
  `);
  return sendEmail({ to: email, subject, html, text });
}
...
export async function sendPaidActivated(email, invoice) {
  ...
  const html = shell('Langganan Aktif', `
    ...
    <p style="margin:0;color:#94a3b8;font-size:13px;">Aktif sampai <strong style="color:#e2e8f0;">${escapeHtml(expires)}</strong>.</p>
  `);
  return sendEmail({ to: email, subject, html, text });
}
